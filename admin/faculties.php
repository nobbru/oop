...
$pageTitle = 'Gerir Faculdades - UP Maputo';
$db = DB::getInstance();
...
if (Input::exists()) {
  if (Token::check(Input::get('token'))) {
    ...
    if ($deleteId > 0) {
      $db->delete('faculties', ['id', '=', $deleteId]);
      Session::flash('home', 'Faculdade removida.');
      Redirect::to('/admin/faculties.php');
    } else if ($name !== '') {
      // create
      $db->insert('faculties', [
        'name' => $name,
      ...
      Session::flash('home', 'Faculdade criada.');
      Redirect::to('/admin/faculties.php');
    } else {
      Session::flash('home', 'Indique o nome da faculdade.');
    }
  }
}

$db->query('SELECT id, name, created_at FROM faculties ORDER BY name');
$rows = $db->results();
...
<div class="space-y-6">
  <h1 class="text-2xl font-semibold text-slate-800">Faculdades</h1>
        ...
        <?php foreach ($rows as $r): ?>
          <tr>
            ...
            <td class="px-4 py-2 text-right">
              <form method="post" onsubmit="return confirm('Remover esta faculdade?');">
